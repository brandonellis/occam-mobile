...
const ClientActivityFeedScreen = ({ route, navigation }) => {
  ...
  const fetchActivities = useCallback(async (targetPage = 1, append = false) => {
    ...
    try {
      ...
      const response = await getClientActivities(clientId, params);
  ...
  }, [clientId, state.activeFilter, state.dateFilter, state.selectedTagIds]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScreenHeader
        title={clientName ? `${clientName}'s Activity` : 'Activity Feed'}
      ...
      {state.loading && !state.refreshing ? (
        ...
        <ActivityEmptyState filterActive={hasActiveFilters} />
      ) : (
        <FlatList
          data={groupedData}
      ...
      {/* Detail sheet with comment composer */}
      <ActivityDetailSheet
        item={selectedItem}
